...
class CGanMnistTrainer(BaseTrain):
        ...
        for epoch in range(epochs):
            ...
            for step in range(1, steps_per_epoch + 1):
                ...
                loss_fake_discriminator = self.discriminator.train_on_batch(
                    generated_images, [fake_label, labels])
                loss_real_discriminator = self.discriminator.train_on_batch(
                    real_images, [real_label, labels])

                g_metric_names = self.g_metric_name()
                loss_generator = self.combined_model.train_on_batch(conditioned_noise, [real_label, labels])
            ...
            if (epoch + 1) % self.config.trainer.pred_rate == 0:
                self.sample_images(epoch, test_data_size)
    ...
    def d_metric_name(is_real=True):
        ...
        d_metric_names = [f'd/loss_{type}', f'd/binary_loss_{type}', f'd/aux_loss_{type}']
        d_accuracy = [f'd/binary_acc_{type}', f'd/aux_acc_{type}']
        return d_metric_names + d_accuracy

    @staticmethod
    def g_metric_name():
        g_metric_names = ['g/loss', f'g/binary_loss', f'g/aux_loss']
        g_accuracy = [f'g/binary_acc', f'aux_acc']
        return g_metric_names + g_accuracy
    ...
    def sample_images(self, epoch, data_size):
        output_dir = f"{self.config.callbacks.predicted_dir}/{epoch + 1}/"
        os.makedirs(output_dir, exist_ok=True)

        save_batch_size = self.config.trainer.pred_save_batch_size
        images = []
        noise = np.random.normal(0, 1, (save_batch_size, 100))
        conditioned_noise = []
        for idx, latent in enumerate(noise):
            one_hot_label = np.zeros((10,), dtype=np.float64)
            np.put(one_hot_label, [idx % 10], [1])
            conditioned_noise.append(np.concatenate((latent, one_hot_label), axis=0))
        ...
        total_images = np.concatenate([np.concatenate(images[i * 10:(i + 1) * 10], axis=0) for i in range(10)], axis=1)
        Image.fromarray(total_images).save(f"{output_dir}/{epoch}.png")
